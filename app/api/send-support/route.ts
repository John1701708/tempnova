...
// 2 alag Resend instances
const resendMark = new Resend(process.env.RESEND_API_KEY);
const resendShahzaib = new Resend(process.env.RESEND_API_KEY_Shahzaib);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const htmlContent = `
      <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#0a0a0f;color:#e2e8f0;border-radius:16px;border:1px solid rgba(255,255,255,0.08);">
        ...
        <div style="background:rgba(255,255,255,0.03);border-radius:12px;padding:16px 20px;border:1px solid rgba(255,255,255,0.06);">
          <p style="margin:0;font-size:12px;color:#64748b;"><span style="color:#475569;font-weight:600;">Received:</span> ${formattedDate}</p>
        </div>
      </div>`;

    const textContent = `From: ${tempEmail}\n\nMessage:\n${message}\n\nReceived: ${formattedDate}`;
    ...
    // Dusri email — shahzaib ke account se shahzaib ko
    const sendShahzaib = resendShahzaib.emails.send({
      from: "TempNova <onboarding@resend.dev>",
      to: ["shahzaibali7940@gmail.com"],
      subject: `TempNova Support — ${tempEmail || "Anonymous"}`,
      html: htmlContent,
      text: textContent,
    });

    // Dono saath mein bhejo
    await Promise.all([sendMark, sendShahzaib]);
